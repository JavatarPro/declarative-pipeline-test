 ...
 */
@Slf4j("logger")
class ConfigInitializationForNodeTest extends Specification {
    ...
    void testCreateEffectiveConfig() {
        def configs = ["init/k8s-pipeline-node.yml"]
        ...
        config.pipeline.service == "chrome-scraper"
        config.pipeline.orchestration == DockerOrchestrationServiceType.K8S
        config.pipeline.build.get(0) == BuildType.NODE
        ...
        config.vcs.url == "ssh://git@gitlab.javatar.com:1022/ats/chrome-scraper.git"
    ...
    void testNpmFlowCreation () {
        def config = "init/k8s-pipeline-node.yml"
